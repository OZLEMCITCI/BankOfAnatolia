...
const Product = () => {
    ...
    return (
        <Container className="product-container">
            <Row> <p className="h2 fw-bold text-center my-xs-1 my-md-1">Anatolia Cards</p></Row>

            <Row className="d-flex justify-content-center ">
                {cardData.map((item) => {

                const { id,title, image, cards, description } = item;
                    return (
                    <Col key={id} xs={12} md={6} lg={4} className=" d-flex justify-content-center">
                        <Card
                          className="card-stil"
                          style={{ width: '22rem', marginBottom: "20px" }}>
                        
<Card.Title className="fw-bold p-3">{ title}</Card.Title>
  
                                                    
                        <Card.Img variant="top" src={image} alt="img" className="product-image"/>
  
                        <ListGroup className="list-group-flush">
                            {cards.map((card) => {
                                return (
                                    <ListGroupItem className="py-1" key={card} className="card-properies-list">
                                    <Card.Link  href="#" className="product-card-link">
                                        <div className="table-parent"> 
                                        <h5 className="table-child">
                                                {card}  </h5>
 ...
 <Card.Text>
    ...
    <Card.Link href="#" className="product-details">Details</Card.Link>
    </div>
  </Card.Body>
</Card>
</Col>)
